...
Scrum\cite{Compendium}\cite{Scrum} is an emerging agile development process which is mostly used in software development.
...
We thought that iterating the same activities over and over would lead to a natural improvement of our process
and at the same time left a lot of possibilities to adjust the requirements and thus the design accordingly.
...
The API defines a set of data models called \textit{things}.
...
The \verb|human/api|\cite{HumanAPI} is a RESTful web API for health data developed by human/api Inc.
Its aim is to offer a single access point to people's health information in order to facilitate development of
health applications. For this purpose, \verb|human/api| collects people's health parameters (blood pressure, weight\ldots)
from a variety of third party services (e.g. Withings) and devices (e.g. sensors) and normalizes it.
The data is then made available through a clean and well documented web API using standardized models
and format (JSON) of which we present an example below.
...
Additionally, \verb|human/api| claims to be HIPAA compliant.
\newline
...
The goal of Open eHealth Foundation \cite{OpenEHealthFoundation} is to create and share
open source software components for the healthcare ICT industry.
...
\verb|human/api| influenced our data models and representation as well providing a good starting
point for discussion on security concerns. HealthVault, together with its SDKs proved to be 
valuable example of a modern health integration platform. We made use of HealthVault SDK to
interact with the Microsoft's platform and were able to re-use a large portion of the code shipped as example.
...
\textbf{Microsoft IIS}\cite{MicrosoftIIS}\newline
...
\textbf{Microsoft WCF}\cite{WCFREST}\newline
...
We don't claim that our choices regarding employed technologies are the 'best' choices possible,
...
REST\cite{REST}\cite{WCFREST} , is an acronym from Representational State Transfer and is basically an architectural
style where clients (user agents) make requests of services to servers.
...
\item Resources can be accessed by client using HTTP methods such as \verb|GET|, \verb|POST|, \verb|PUT| and \verb|DELETE|.
      Each of these methods corresponds to an operation:
			\begin{itemize}
				...
				\item \verb|PUT|: updates a collection or a particular element in it.
...
Because an element can only be updated or deleted once.
...
\subsection{SOAP}
\label{subsec:soap}
SOAP\cite{SOAP} stands for Simple Object Access Protocol. As the name suggests it is a protocol for
exchanging information between web services in a network.
...
In this section we present some of the testing frameworks explored during our preliminary study.

\subsection{JUnit}

JUnit\cite{JUnit} is a Java framework for writing tests. It is also a good tool when using test driven development.
...
Jasmine \cite{Jasmine} is a framework for testing JavaScript code.	
