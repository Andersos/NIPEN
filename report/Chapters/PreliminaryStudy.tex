...
%Because each phase needs to be perfected and completed before moving on to the next one,
%changes to project's requirements are not allowed.
...
The person in charge of populating the product backlog is the \emph{product owner}.
...
%From a technical perspective, HealthVault is an XML based web-service.
%As such, it exposes an XML based web API that developers can leverage
%to easily build HealthVault-enabled applications.

%Health records can be from a number different third-party sources, which in
%HealthVault's terminology are referred to as \textit{apps}, such as:
%\begin{itemize}
%\item smartphone and desktop applications
%\item devices like step counters, blood glucose meters, weighting scales\ldots
%\item third-party health services like Withings, \ldots
%\end{itemize}
%and shared with other people like family members, doctors and healthcare providers.
...
%human/api takes care of collecting such information from a number of third-party services
%and devices (e.g. sensors) and normalizing it.

%to fetch health information
%from a variety of sensors, devices and other third-party services.
...
%The data itself is pulled by human/api system from a number of third-party services
%and devices and normalized.
